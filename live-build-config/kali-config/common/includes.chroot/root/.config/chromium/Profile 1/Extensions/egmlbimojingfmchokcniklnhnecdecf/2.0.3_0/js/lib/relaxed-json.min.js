(function () {
            ...
            function findToken() {
                return some(tokenSpecs, function (tokenSpec) {
                    ...
                    if (m) {
                        ...
                        return {raw: raw, matched: tokenSpec.f(m, line)}
                    }
                })
            }
    ...
    function fStringSingle(m) {
        ...
        return {type: "string", match: '"' + content + '"', value: JSON.parse('"' + content + '"')}
    }

    function fStringDouble(m) {
        return {type: "string", match: m[0], value: JSON.parse(m[0])}
    }

    function fIdentifier(m) {
        return {
            type: "string", value: m[0], match: '"' + m[0].replace(/./g, function (c) {
                return c === "\\" ? "\\\\" : c
            }) + '"'
        }
    }
    ...
    function fComment(m) {
        return {
            type: " ", match: m[0].replace(/./g, function (c) {
                return /\s/.test(c) ? c : " "
            })
        }
    }
    ...
    function fNumber(m) {
        return {type: "number", match: m[0], value: parseFloat(m[0])}
    }
    ...
    function fKeyword(m) {
        ...
        return {type: "atom", match: m[0], value: value}
    }
    ...
    function makeTokenSpecs(relaxed) {
        ...
        var ret = [{re: /^\s+/, f: f(" ")}, {re: /^\{/, f: f("{")}, {re: /^\}/, f: f("}")}, {
            re: /^\[/,
            f: f("[")
        }, {re: /^\]/, f: f("]")}, {re: /^,/, f: f(",")}, {re: /^:/, f: f(":")}, {
            re: /^(true|false|null)/,
            f: fKeyword
            ...
            if (token.type === "]" || token.type === "}") {
                ...
                if (commaI && res[commaI].type === ",") {
                    ...
                    if (preCommaI && res[preCommaI].type !== "[" && res[preCommaI].type !== "{") {
                        res[commaI] = {type: " ", match: " ", line: tokens[commaI].line}
                    }
                }
            }
            res.push(token)
    ...
    function skipColon(tokens, state) {
        ...
        if (colon.type !== ":") {
            ...
            if (state.tolerant) {
                state.warnings.push({message: message, line: colon.line});
                state.pos -= 1
            } else {
    ...
    function skipPunctuation(tokens, state, valid) {
        ...
        while (true) {
            if (valid && valid.indexOf(token.type) !== -1) {
                ...
                if (state.tolerant) {
                    state.warnings.push({message: message, line: token.line});
    ...
    function parsePair(tokens, state, obj) {
        ...
        if (token.type !== "string") {
            ...
            switch (token.type) {
                case":":
                    token = {type: "string", value: "null", line: token.line};
                    state.pos -= 1;
                    break;
                case"number":
                case"atom":
                    token = {type: "string", value: "" + token.value, line: token.line};
                    break;
                case"[":
                case"{":
                    state.pos -= 1;
    ...
    function parseMany(tokens, state, obj, opts) {
        ...
        if (token.type === "eof") {
            ...
            token = {type: opts.endSymbol, line: token.line}
        }
        switch (token.type) {
            case opts.endSymbol:
                return obj;
        ...
        while (true) {
            ...
            if (token.type !== opts.endSymbol && token.type !== ",") {
                ...
                token = {type: token.type === "eof" ? opts.endSymbol : ",", line: token.line};
                state.pos -= 1
            }
            switch (token.type) {
                case opts.endSymbol:
    ...
    var RJSON = {transform: transform, parse: parse, stringify: stringify};
